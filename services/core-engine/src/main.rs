...
struct PrefetchResponse {
    status: String,
    keys_prefetched: usize,
    predicted_keys: Vec<String>,
}
...
async fn cache_set(
    ...
    let size = serde_json::to_string(&req.value)
        .map(|s| s.len() as u64)
        .unwrap_or(0);
    let full_key = format!("{}:{}", ns, req.key);
    {
        ...
        store.insert(
            ...
            CacheEntry {
                value: req.value,
                ttl_secs: ttl,
                created_at: s.start_time.elapsed().as_secs(),
            },
        );
    }
    {
...
async fn prefetch(
    ...
    let predicted: Vec<String> = req
        .keys
        .iter()
        .take(3)
        .map(|k| format!("{}_next", k))
        .collect();
    s.stats.lock().unwrap().prefetch_hits += count as u64;
    Json(PrefetchResponse {
        status: "ok".into(),
        keys_prefetched: count,
...
async fn stats(State(s): State<Arc<AppState>>) -> Json<CacheStats> {
    ...
    let hit_rate = if total_lookups > 0 {
        st.hits as f64 / total_lookups as f64
    } else {
        0.0
    };
    let prefetch_accuracy = if st.total_gets > 0 {
        (st.prefetch_hits as f64 / st.total_gets as f64).min(1.0)
    } else {
        0.0
    };
    Json(CacheStats {
        total_gets: st.total_gets,
        total_sets: st.total_sets,
        total_deletes: st.total_deletes,
        hits: st.hits,
        misses: st.misses,
        hit_rate,
